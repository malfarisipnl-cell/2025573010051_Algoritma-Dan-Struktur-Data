...
class Queue {
    constructor() {
        this.head = null;  // antrian dari depan (dari sini)
        this.tail = null;  // antrian dari belakang ( ke sini)
        this.size = 0;
    }

    enqueue (data){         // menambah kebelakang - O(1)
    ...
    dequeue() {     // menghapus dari belakang - O(1)
        if (this.isEmpty()) return  null;
        const val = this.head.data;
        this.head = this.head.next;
        if (!this.head) this.tail = null;  //membuat queque menjadi  kosong
        this.size--;
        return val;
    }
...
// --- bagian APlikasi : BPS pada Grid ----------
// BPS = Breadth-Frist Search): Belajar graoh/grid level per level
// memakai Queue untuk memasukan tetangga, proses berurutan

function bfsGrid(grid, startR, startC) {
    const rows = grid.length, cols = grid[0]. length;
    const visited = Array.from ({length:rows}, ()=>Array(cols).fill(false));
    const queue = new Queue();
    const arah = [[0,1],[0,-1],[1,0],[-1,0]];  // arah kanan,kiri,bawah,atas
...
console.log('\n=== BSF pada Grid (. = bisa jalan, # = tembok) ===');
const grid = [
    ['.','.','.','#','.'],
    ['.','#','.','#','.'],
    ['.','#','.','.','.'],
